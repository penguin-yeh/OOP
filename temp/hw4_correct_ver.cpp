...
class Rep_payload : public payload {
    ...
    public:
        ...
        // Hello_header is derived from payload_generator to generate a payload
...
void add_initial_event3 (unsigned int src, unsigned int dst, unsigned t = 0, string msg = "default"){ // t¨Obroadcast time
...
double Hash_function(int id, double &x, double &y, unsigned int x_max, unsigned int y_max) {
	// id = dst_id
	// x,y = ≠nøÈ•X™∫ 
	//  
	std::string s = std::to_string(id);
	std::hash<string> hasher;
	unsigned int v1 = hasher(s);
	
	std::string s1 = std::to_string(v1);
	unsigned int v2 = hasher(s1);
...
void GR_node::recv_handler (packet *p){
    ...
    // ≠Y¶¨®ÏRes±qqueueÆ≥•Xº»¶s°A®√∞µßπ°C 
    ...
    else if(p->type() == "Rep_packet" && !hi) { // des to deadend
    	//cout << "Rep" << endl;
    	//cout << p->type() <<endl;
    	p3 = dynamic_cast<Rep_packet*> (p);
		int target_id; // send to target_id
        int src_id = p3 -> getHeader() -> getSrcID();
    	// find deadend position
    	
    	double deadend_x, deadend_y;
        Hash_function(src_id, deadend_x, deadend_y, Xmax, Ymax);
    	int now_id = getNodeID(); // get dstID
    	// x,y for now
    	double x = (dynamic_cast<GR_node*>(node::id_to_node(now_id)))->getX();
        double y = (dynamic_cast<GR_node*>(node::id_to_node(now_id)))->getY();
    	// 
    	double min_dis = dis(deadend_x,deadend_y,x,y); // ∫‚•XdeadendªP∑Ì´e¬I™∫∂Z¬˜ 
    	double origin = min_dis; // deadendªP∑Ì´e¬I™∫∂Z¬˜ 
        for(map<unsigned int,bool>::const_iterator it = nblist.begin(); it != nblist.end(); it ++) {
            double x1 = (dynamic_cast<GR_node*>(node::id_to_node(it -> first)))->getX();
            double y1 = (dynamic_cast<GR_node*>(node::id_to_node(it -> first)))->getY();
            if(dis(deadend_x,deadend_y,x1,y1) < min_dis){ // if the distance is closer?
                min_dis = dis(deadend_x,deadend_y,x1,y1);
                target_id = it -> first; // change the closer id
            } // if()
        } // for()
    	if(origin == min_dis){//
			node_co[src_id] = {(dynamic_cast<GR_node*>(node::id_to_node(src_id)))->getX(),(dynamic_cast<GR_node*>(node::id_to_node(src_id)))->getY()}; 
			return;
        } //if()
        p3->getHeader()->setPreID ( getNodeID() );//pre -> now id
        p3->getHeader()->setNexID ( target_id );//nex -> target_id
        send_handler(p3);
	} // else if(Rep)
	else if(p->type() == "Ret_packet" && !hi) { // src to deadend
		// p4 for Ret
		//cout << p->type() <<endl;
		p4 = dynamic_cast<Ret_packet*> (p);
 		double deadend_x,deadend_y,x,y,x1,y1,x2,y2;
 		double min_dis,origin;
 		int target_id,src_id,now_id;
 
        src_id = p4 -> getHeader() -> getSrcID(); // src_id src'id
    	// deadend
        Hash_function(p4->getHeader()->getDstInfo(), deadend_x, deadend_y, Xmax, Ymax); // deadend ¶Ï∏m 
        
    	now_id = getNodeID(); // x,y for now
    	x = (dynamic_cast<GR_node*>(node::id_to_node(now_id)))->getX();
        y = (dynamic_cast<GR_node*>(node::id_to_node(now_id)))->getY();
    	min_dis = dis(deadend_x,deadend_y,x,y); // deadendªP∑Ì´e¬I™∫∂Z¬˜ (0 deadend∂Z¬˜) 
    	origin = min_dis; // deadendªP∑Ì´e¬I™∫∂Z¬˜
        ...
        for(map<unsigned int,bool>::const_iterator it = nblist.begin(); it != nblist.end(); it ++) {
            ...
            if(dis(deadend_x,deadend_y,x1,y1) < min_dis) { // if the distance is closer?
                //¶≥ßÛ™Ò™∫¥NßÛ∑s 
				min_dis = dis(deadend_x,deadend_y,x1,y1);
                p4->getHeader()->setNexID ( it->first ); // change the closer id
            } // if()
        } // for() 
        //============== 
    	if(origin == min_dis){ // •N™Ìß‰®ÏDeadend 
    		//cout << "it should be here2  " <<  getNodeID() <<"   " << node_co.size() <<endl;
			// •N™Ìß‰®Ï¬˜deadend≥Ã™Ò™∫¬I°A±N®‰¶s∞_®” 
			// •Õ¶®¶p™G¶b¶C™Ì§§¶≥ß‰®ÏDst∏ÍÆ∆°A¶^∂«Res_packet°A¶p™G®S¶≥¥N§£•Œ∞ ß@ 
			bool find = false; // find = false ®S¶≥Æyº–∏ÍÆ∆ 
			for(map<unsigned int,pair<double,double>>::const_iterator it = node_co.begin(); it != node_co.end(); it ++) {
            	// ß‰¨›¨›¶≥®S¶≥dst∏ÍÆ∆ 
	            int dst = it->first; // get dst's id
	            if( dst == p4 -> getHeader() -> getDstInfo() ) find = true;
        	} // for()
			if( find == true ) { // ¶≥ß‰®Ïdst ¶^∞eres_packet 
				//cout << "it must here" <<endl;
				packet *p5 = dynamic_cast<Res_packet*> ( packet::packet_generator::generate("Res_packet") );
				packet *hdr = dynamic_cast<Res_packet*> (p5->getHeader() );
				packet *pld = dynamic_cast<Res_packet*> (p5->getPayload() );
				//hdr->setDstX( (dynamic_cast<GR_node*>(node::id_to_node(getNodeID())))->getX() );
				//hdr->setDstY( (dynamic_cast<GR_node*>(node::id_to_node(getNodeID())))->getY()  );
				// ¶^∞eRes_packetµπSrcNode 
				p5->getHeader()->setSrcID( getNodeID() );
				p5->getHeader()->setPreID( getNodeID() );
				// ∫‚≥Ã™Ò™∫¬I 
				x2 = (dynamic_cast<GR_node*>(node::id_to_node(p4->getHeader()->getSrcID())))->getX();
				y2 = (dynamic_cast<GR_node*>(node::id_to_node(p4->getHeader()->getSrcID())))->getY();
				
				x = (dynamic_cast<GR_node*>(node::id_to_node(getNodeID() )))->getX();
				y = (dynamic_cast<GR_node*>(node::id_to_node(getNodeID() )))->getY();
		        ...
		        p5->getHeader()->setSrcID(getNodeID()); 
		        p5->getHeader()->setPreID(getNodeID());
		        p5->getHeader()->setNexID(target_id);
				p5->getHeader()->setDstInfo(p4 -> getHeader() -> getDstInfo());//≠n∞›•ÿ™∫¶a™∫data 
				p5->getHeader()->setDstID(p4 -> getHeader() -> getSrcID());
				send_handler(p5);//Res 
				packet::discard(p5);
				return;
			} //if()
        ...
        // ß‰®ÏßÛ™Ò™∫deadend°Aƒ~ƒÚ∞eRes_packet 
        p4->getHeader()->setPreID ( getNodeID() ); //pre -> now id
        //p4->getHeader()->setNexID ( target_id );   //nex -> target_id
        send_handler(p4);
	} // else if(Ret)
	else if(p->type() == "Res_packet" && !hi) { // deadend to src
		// use greedy to Src
		// p5 for Res_packet
		//cout << p->type() <<endl;
		p5 = dynamic_cast<Res_packet*> (p);
		int target_id,src_id,dst_id,now_id;
		double x,y,x2,y2,x1,y1;
		double min_dis,origin;
		
		src_id = p5->getHeader()->getSrcID();
		dst_id =  p5->getHeader()->getDstID();
		
		x2 = (dynamic_cast<GR_node*>(node::id_to_node(dst_id)))->getX();
		y2 = (dynamic_cast<GR_node*>(node::id_to_node(dst_id)))->getY();
		now_id = getNodeID();
		x = (dynamic_cast<GR_node*>(node::id_to_node(now_id)))->getX();
		y = (dynamic_cast<GR_node*>(node::id_to_node(now_id)))->getY();
		
		min_dis = dis(x2,y2,x,y);
		origin = min_dis;
    	// ∫‚•X∑Ì´eªP•ÿ™∫¶a∂Z¬˜ 
    	...
    	for(map<unsigned int,bool>::const_iterator it = nblist.begin(); it != nblist.end(); it ++) {
            ...
            x1 = (dynamic_cast<GR_node*>(node::id_to_node(it -> first)))->getX();
            y1 = (dynamic_cast<GR_node*>(node::id_to_node(it -> first)))->getY();
            if(dis(x2,y2,x1,y1) < min_dis){ // if the distance is closer?
                min_dis = dis(x2,y2,x1,y1);
                target_id = it -> first; // change the closer id
            } // if()
        } // for()
           
    	if( origin == min_dis ){ // •N™Ì¶^®Ï§FSrc
    	    p2 = dynamic_cast<GR_packet*> (cache.front());
    	    cache.pop();
    	    x2 = (dynamic_cast<GR_node*>(node::id_to_node(p2->getHeader()->getDstID())))->getX();
			y2 = (dynamic_cast<GR_node*>(node::id_to_node(p2->getHeader()->getDstID())))->getY();
			now_id = getNodeID();
			x = (dynamic_cast<GR_node*>(node::id_to_node(now_id)))->getX();
			y = (dynamic_cast<GR_node*>(node::id_to_node(now_id)))->getY();
			min_dis = dis(x2,y2,x,y);
            ...
            //cout << it->first << endl;
	            ...
	            if(dis(x2,y2,x1,y1) < min_dis){ // if the distance is closer?
	                min_dis = dis(x2,y2,x1,y1);
	                p2->getHeader()->setNexID(it->first); // change the closer id
	            } // if()

        	} // for()
    	    p2->getHeader()->setPreID(getNodeID());
			//p2->getHeader()->setNexID(target_id);
