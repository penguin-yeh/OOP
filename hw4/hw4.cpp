...
void GR_node::recv_handler (packet *p){
    ...
    if (p->type() == "GR_packet" && !hi)
    {
        ...
        if( p2->getHeader()->getDstID() == p2->getHeader()->getDstInfo() )//actually know dst's coor
        {
        ...
        else if( p2->getHeader()->getDstID() != p2->getHeader()->getDstInfo() )//doesn't have dst information -> send Ret_pkt
        {
    ...
    else if(p->type() == "Res_packet" && !hi)
    {
            ...
            if(min == dis(getNodePos( getNodeID() ), getNodePos( p9->getHeader()->getDstID() )))//dead end->src, send GR
            {
