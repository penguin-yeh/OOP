...
// int getNeighbors(unsigned int id)
// {
//     GR_node* ptr = dynamic_cast <GR_node*> (node::id_to_node(id));
//     return ptr->one_hop_neighbors.size();
// }
...
void node::send (packet *p){ // this function is called by event; not for the user
    ...
    if (p == nullptr) return;
    //cout << "---------------"  << endl;
    //GR_node* ptr = dynamic_cast <GR_node*> (this);
    if(p->type() == "GR_packet")
    {
        unsigned int _nexID = p->getHeader()->getNexID();
        for (int i=0; i<node::getNodeNum() ;i++) {
            unsigned int nb_id = i; // neighbor id

            //if(nb_id == getNodeID() || link::id_id_to_link(getNodeID(), nb_id) == nullptr)  continue;
...
void GR_node::recv_handler (packet *p){
    ...
    if (p->type() == "GR_packet"){
        ...
        //this->one_hop_neighbors.insert(pair<unsigned,bool>(p3->getHeader()->getSrcID(), true));
        ...
        if(this->getNodeID() == p3->getHeader()->getSrcID())
        {
            ...
            //cout << "size is " << this->one_hop_neighbors.size() << endl;
            // you cannot use node::getPhyNeighbors() since it is private now
    }
...
int main()
{
    ...
    for(int i=0; i<node_cnt;i++)
    {
        for(int j=0; j<node_cnt; j++)
        {
            ...
            {
                link::link_generator::generate("simple_link", i, j);
            }
        }
    }
